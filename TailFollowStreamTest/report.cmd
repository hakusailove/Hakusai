...
REM 対象設定
set TARGET=TailFollowStreamTest.dll

"%OPENCOVER%" -register:user -target:"%NUNITCONS%" -targetargs:"%TARGET%" "-filter:+[*]* -[*Test]* -[*]*.NamespaceDoc" -output:"%COVXML%"
"%REPORTGEN%" "-reports:%COVXML%" "-targetdir:%TARGETDIR%" -filters:+TailFollowStream